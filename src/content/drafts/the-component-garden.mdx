...
Welcome to [The Component Garden](https://github.com/antropia-studio/TheComponentGarden), a playground to build innovative user interfaces, components and animations. This is our way to keep ourselves updated with the latest design trends, while keeping a critical eye on the usability and accessibility of our components.
...
By imagining what it'd look like for five cards, we get a better feeling of the maths behind the component.
...
The animation here is more tricky, and we can basically split it into two different problems: the animation while the user is dragging the cards, and the animation when restoring their position.

For the dragging gesture we used the omni-present [`react-native-gesture-handler`](https://docs.swmansion.com/react-native-gesture-handler/docs/) and its [`Pan` gesture](https://docs.swmansion.com/react-native-gesture-handler/docs/gestures/pan-gesture). We paired it with [`react-native-reanimated`](https://docs.swmansion.com/react-native-reanimated/) to achieve a smooth and spring-y animation. The idea here is to keep a `dragProgress` shared value that we then send to each item for them to calculate their exact position. I included an overly-documented snippet here with the way we calculate shared values:
...
In this post we went through the implementation of our first _crop_ for The Component Garden. We will keep using this project to explore advanced UI and animation techniques, and we will use it build them in public so that others can learn with us.
